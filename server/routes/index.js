...
exports.register = function(db) {
	...
	alerts.ensureIndex( { "location.coordinates" : "2dsphere" } );
    users.ensureIndex( { "location.coordinates" : "2dsphere" } );
    // Submit to the DB
    ...
    users.insert({
    	...
    	if (err) {
            ...
            alerts.find({$near : { $geometry : { type: "Point", coordinates : [lng, lat]}, $maxDistance : 1000}}, function(err, docs) {
